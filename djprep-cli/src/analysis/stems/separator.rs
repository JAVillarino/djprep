...
impl OrtStemSeparator {
    ...
    fn run_inference(&self, input_path: &Path, stem_paths: &StemPaths) -> Result<()> {
        ...
        for (i, chunk) in chunks.iter().enumerate() {
            ...
            let input_tensor =
                Tensor::from_array(input_data).map_err(|e| DjprepError::StemUnavailable {
            ...
            let output = outputs.iter().next().map(|(_, v)| v).ok_or_else(|| {
    ...
    fn write_stereo_wav(&self, path: &Path, audio: &StereoBuffer) -> Result<()> {
        ...
        writer.finalize().map_err(|e| DjprepError::OutputError {
            path: path.to_path_buf(),
