...
fn test_pipeline_produces_valid_json() {
    ...
    assert!(
        bpm.get("confidence").is_some(),
        "BPM should have confidence"
    );
    ...
    assert!(
        key.get("standard").is_some(),
        "Key should have standard notation"
    );
    assert!(key.get("open_key").is_some(), "Key should have open_key");
    assert!(
        key.get("confidence").is_some(),
        "Key should have confidence"
    );
}
